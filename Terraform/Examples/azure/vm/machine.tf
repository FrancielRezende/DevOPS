...
resource "azurerm_network_interface" "az-net-interface" {
...
resource "azurerm_network_interface_security_group_association" "az-nsg-association" {
  network_interface_id      = azurerm_network_interface.az-net-interface.id
  network_security_group_id = data.terraform_remote_state.vnet.outputs.terraform-netwoek-security-group-id
}
resource "azurerm_linux_virtual_machine" "az-vm-ubuntu" {
  name                  = var.az_vm_name
  resource_group_name   = azurerm_resource_group.az-resource-group.name
  location              = var.az_location
  size                  = var.az_vm_size
  admin_username        = var.az_vm_admin
  network_interface_ids = [azurerm_network_interface.az-net-interface.id]
  admin_ssh_key {
